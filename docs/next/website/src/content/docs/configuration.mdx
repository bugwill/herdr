...
On Unix, Herdr supports `kitten transfer` uploads and downloads through a compatible outer terminal such as Kitty. Kitty still handles the permission prompt. Transfers stay attached to their original pane when focus changes, and require no Herdr setting. For example, download a file from a Herdr pane to the outer terminal's machine:
